...
internal object PreferencesManager {
    ...
    internal val secureListenerStore =
    ...
    internal fun set(key: String, value: String) {
        val editor: SharedPreferences.Editor = preferences?.edit()
            ?: throw IllegalStateException("Attempting to put values into null preferences. Maybe the plugin is not correctly configured?")
    ...
    internal fun addSubscription(lambda: (SharedPreferences, String?) -> Unit) {
        SharedPreferences.OnSharedPreferenceChangeListener(lambda).let {
            secureListenerStore.add(it)
